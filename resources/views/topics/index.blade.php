        ...
        <li class="flex-col flex">
            ...
            >
                ...
                <div class="flex flex-row items-center border-l border-neutral-300 px-2 w-72">
                    <img class="flex h-8 w-8 rounded"
                        src="https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=facearea&facepad=2&w=256&h=256&q=80"
                        alt="">
                    <div class="flex flex-col mx-1">
                        <a href="{{route('posts.show', $topic->posts[0]->id)}}">
                            <span class="flex font-serif text-sm text-slate-500">{{Str::limit($topic->posts[0]->title, 30)}}</span>
                        </a>
                        <span class="flex font-serif text-xs text-slate-400">{{$topic->posts[0]->comments[0]->published_at}}</span>
                    </div>
                </div>
                <div class="flex flex-col justify-center border-l border-neutral-300 px-2 w-24">
                    <span class="flex font-serif text-xs text-slate-500">Posts: {{count($topic->posts)}}</span>
                    <span class="flex font-serif text-xs text-slate-500">Comments: {{count($topic->comments)}}</span>
                </div>
            </div>
        </li>
        @endforeach
